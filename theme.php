...
class BootstrapTheme extends Theme {
  ...
  public function add_template_vars() {
    if (!$this->template_engine->assigned('pages')) {
      ...
      foreach ($pages as $page) {
        if ($page->slug == substr($_SERVER['REQUEST_URI'], 1)) {
          $page->onPage = TRUE;
        }
      }
  ...
  public function filter_post_tags_list($tags) {
    ...
    $tag_list = array();
    foreach ($tags as $tag) {
      $tag_list[] = sprintf('<a href="%s" rel="tag" class="label label-info">%s</a>', URL::get('display_entries_by_tag', array('tag' => $tag->term)), $tag->term_display);
    }
  ...
  public function action_form_comment($form) {
    ...
    $form->cf_fieldset->append('wrapper', 'cf_commentarea');

    $form->cf_fieldset->cf_inputarea->class = 'span2';
    $form->cf_fieldset->cf_commentarea->class = 'span3';
    ...
    $form->cf_content->move_into($form->cf_fieldset->cf_commentarea);
    $form->cf_content->caption = _t('Your Comment');
    $form->cf_content->placeholder = _t('Your Comment');
  ...
  public function theme_header($theme) {
    Stack::add('template_stylesheet', array(Site::get_url('theme') . '/bootstrap/css/bootstrap.min.css', 'screen'));
    Stack::add('template_stylesheet', array(Site::get_url('theme') . '/bootstrap/css/bootstrap-responsive.min.css', 'screen'));
  ...
  /**
   * Add a project_progress block to the list of available blocks.
   */
  public function filter_block_list($block_list) {
    $block_list['progress'] = _t('Progress bar');
    return $block_list;
  }
   ...
   */
  public function action_block_form_progress($form, $block) {
    $content = $form->append('text', 'percent', $block, _t('% complete:'));
  }

  /**
   * Produce a progress block.
   *
   * @param Block $block The block instance to be configured
   * @param Theme $theme The active theme
   */
  public function action_block_content_progress($block, $theme) {
    return $block;
  }
